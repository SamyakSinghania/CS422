...
Mipc::MipcDumpstats()
{
  ...
  l.print ("Number of syscalls: %llu", num_syscalls);
  l.print ("Number of load delays: %llu", num_load_delays);
  l.print ("");
...
Mipc::Reboot (char *image)
{
   ...
   if (image) {
     ...
     // _insDone = TRUE;
      ...
      NOP._bdslot = 0;
      NOP._btaken = 0;
      NOP._btgt = 0xdeadbeef;
      NOP._opControl = func_sll;
      IF = ID = EX = MEM = WB = IF_ID = ID_EX = EX_MEM = MEM_WB = NOP;
